 ...
 * 生产者
 * Class Publisher
 * @package Zhan3333\RabbitMQ\TaskPublishers
 */
class TaskPublisher extends RabbitMQ
{
    /**
     * 推送消息
    ...
    {
        $this->logger->debug('Task publish start', [$message]);
        if (empty($message)) {
        ...
        try {
            ...
            throw new RabbitMQPublishException('Task publish message failed', 0, $exception);
        }
    }

    public function __destruct()
    {
        $this->logger->info('Task publish end');
        parent::__destruct();
    }
}
